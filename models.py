...
class SINR_Net(torch.nn.Module):
    ...
    def __init__(self, input_len=4, hidden_dim=256, dropout=0.5, layers=4):
        ...
        self.location_encoder = torch.nn.Sequential(
            ...
            torch.nn.ReLU(),
            *[ResidLayer(hidden_dim, dropout) for _ in range(layers)]
        )
    ...
    def _calculate_val_metrics(self, log_dict, device):
        ...
        if self.params.model.startswith("sat_sinr_mf_"):
            for i in range(len(self.net.net.resid_l)):
                log_dict["embedder_stats/" + str(i) + "_" + "weight_mean"] = self.net.net.resid_l[
                    i].embedder.weight.abs().mean()
                log_dict["embedder_stats/" + str(i) + "_" + "weight_std"] = self.net.net.resid_l[
                    i].embedder.weight.abs().std()
                log_dict["embedder_stats/" + str(i) + "_" + "bias_mean"] = self.net.net.resid_l[
                    i].embedder.bias.abs().mean()
                log_dict["embedder_stats/" + str(i) + "_" + "bias_std"] = self.net.net.resid_l[
                    i].embedder.bias.abs().std()
...
class SAT_SINR(SINR):
    ...
    def __init__(self, params, dataset, sent2_net, **kwargs):
        ...
        self.reds = torch.utils.data.DataLoader(reds, shuffle=True, batch_size=params.dataset.batchsize,
...
class SASI_MF(torch.nn.Module):
    ...
    def __init__(self, params, mode=""):
        ...
        self.net = Context_SINR_Net(inp_size, hidden_dim=params.sinr_hidden, dropout=params.dropout,
                                    layers=params.sinr_layers, mode=mode)
        self.predictors = params.dataset.predictors
...
class SASI_EF(torch.nn.Module):
    ...
    def __init__(self, params, enc_dim=24):
        ...
        self.net = SINR_Net(24 + enc_dim, hidden_dim=params.sinr_hidden, dropout=params.dropout,
                            layers=params.sinr_layers)
