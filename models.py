...
class SAT_SINR(SINR):
    ...
    def __init__(self, params, dataset, sent2_net, **kwargs):
        ...
        self.dataset = dataset
        # Instantiate random_europe_dataloader
    ...
    def apply_model_and_an_full_loss(self, batch, dataset, params):
        ...
        rand_pred = torch.sigmoid(self.net((random_loc_features.to(loc_features.device), random_sent2.to(loc_features.device))))
...
class SASI_MF(torch.nn.Module):
    """Only SINR adaption to not use SINR_Net."""
    def __init__(self, params):
        ...
        if "env" in params.dataset.predictors:
            inp_size += 20
        self.net = Context_SINR_Net(inp_size, hidden_dim = params.sinr_hidden, dropout = params.dropout, layers = params.sinr_layers)
        self.predictors = params.dataset.predictors
...
class SASI_EF(torch.nn.Module):
    ...
    def __init__(self, params, enc_dim = 24):
        ...
        if "env" in params.dataset.predictors:
            inp_size += 20
        self.net = SINR_Net(inp_size + enc_dim, hidden_dim = params.sinr_hidden, dropout = params.dropout, layers = params.sinr_layers)
